...
class LocalEmbeddingModelManager {
    ...
    private async getRootPath(): Promise<string> {
        ...
        return rootPath.includes("/aiverse/")
            ? rootPath.split("/aiverse/")[0] + "/aiverse/"
    ...
    private async initializeModel(): Promise<void> {
        ...
        try {
            ...
            aiverseLogger.debug("Initializing BGE embedding model...");
            ...
            aiverseLogger.debug("BGE model initialized successfully");
        } catch (error) {
            aiverseLogger.error("Failed to initialize BGE model:", error);
            throw error;
        }
    }
    ...
    public async generateEmbedding(input: string): Promise<number[]> {
        ...
        try {
            ...
            aiverseLogger.debug("Raw embedding from BGE:", {
            ...
            aiverseLogger.error("Embedding generation failed:", error);
            throw error;
        }
    }
